(function ($) {
    ...
    var getLevel = function (item, that) {
        var _level = 0;
        var _parentId = item.parentId;
        while (_parentId !== that.options.treeRootLevel) {
            _level++;
            _parentId = getNodeById(_parentId, that.options.data).parentId;
        }
        return _level;
    };

    var getNodeById = function (id, data) {
            ...
            $.each(this.header.fields, function (j, field) {
                ...
                if (column.checkbox || column.radio) {
                    ...
                    if (that.options.treeView && column.field === that.options.treeField) {
                        var indent = item.parentId === that.options.treeRootLevel ? '' : sprintf('<span style="margin-left: %spx;"></span>', getLevel(item, that) * 20);
    ...
    $.extend($.fn.bootstrapTable.defaults, {
        ...
        collapseIcon: "fa fa-lg fa-caret-right",//折叠样式
        expandIcon: "fa fa-lg fa-caret-down"//展开样式
    });
})(jQuery);