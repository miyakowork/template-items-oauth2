...
public class ResourceModuleRestController extends BaseRestController {
     ...
     * 添加新资源模块
     *
     * @param oauthResourceModule 添加的资源模块
     * @return R
     */
    @RequestMapping("add")
    public R add(OauthResourceModule oauthResourceModule) {
        try {
            if (resModuleService.save(oauthResourceModule, OauthResourceModule.class))
                return R.ok("添加成功");
            else {
                return R.ok();
            }
     ...
     * 修改资源模块
     *
     * @param oauthResourceModule 修改的资源模块
     * @return R
     */
    @RequestMapping("edit")
    public R edit(OauthResourceModule oauthResourceModule) {
        try {
            if (resModuleService.edit(oauthResourceModule, OauthResourceModule.class))
